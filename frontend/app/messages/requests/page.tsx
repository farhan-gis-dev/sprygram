...
export default function MessageRequestsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([meProfile, response]) => {
        const requests = (response.items || []).filter((entry) => entry.unreadCount > 0 && entry.peer.followStatus !== 'accepted');
        setMe(meProfile);
        setConversations(requests);
        setSelectedPeerId(requests[0]?.peer.userId || null);
      })
  ...
  const loadThread = async (peerUserId: string, cursor?: string | null) => {
    if (!cursor) setThreadLoading(true);

    try {
      const response = await sprygramApi.getThreadMessages(peerUserId, { limit: 30, cursor: cursor || undefined }, auth);
      setThread((previous) => (cursor ? [...response.items, ...previous] : response.items || []));
      setThreadCursor(response.nextCursor);
    } catch (error: any) {
      notifications.show({ color: 'red', title: 'Requests error', message: error.message });
    } finally {
  ...
  if (!isReady || loading) {
    return <LoadingState message="Loading message requests..." />;
  }
      ...
      <aside className="w-[360px] border-r border-border">
        <div className="border-b border-border px-4 py-3">
          <Group justify="space-between">
            <Stack gap={0}>
              <Text fw={700}>Message requests</Text>
              <Text size="xs" c="dimmed">{me?.username || 'Sprygram'} · Review incoming chats</Text>
            </Stack>
            <Link href="/messages" className="rounded-md p-1 hover:bg-gray-100" aria-label="Back to messages">
        ...
        <div className="hide-scrollbar h-[calc(100%-88px)] overflow-y-auto p-2">
          {visibleConversations.length === 0 ? (
            <Text size="sm" c="dimmed" px={8} py={8}>No message requests.</Text>
              ...
              <Group wrap="nowrap" justify="space-between">
                <Group wrap="nowrap">
                  ...
                  <Stack gap={1} className="min-w-0">
                    ...
                    <Text size="xs" c="dimmed" lineClamp={1}>{summarizeRichMessageContent(conversation.lastMessage?.content) || 'Tap to review request'}</Text>
                  </Stack>
                </Group>
      ...
      <section className="flex flex-1 flex-col">
        {!selectedConversation ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <Text fw={700} size="lg">No request selected</Text>
            <Text size="sm" c="dimmed">Open a request to review the conversation.</Text>
          </div>
        ) : (
          <>
            <div className="border-b border-border px-4 py-3">
            ...
            <div className="hide-scrollbar flex-1 overflow-y-auto p-4">
              ...
              {threadLoading ? <LoadingState message="Loading request..." /> : null}
